"""Smoke test for the seqcc toolchain driver (sub-phase T1).

Verifies that the freshly-built `seqcc` executable can be invoked and
responds to --help and --version.  This is intentionally minimal: real
pipeline tests arrive in sub-phase T2 with
...
class SeqccSmokeTest(unittest.TestCase):
    ...
    def test_unknown_input_returns_two(self):
        """Passing an input file in T1 returns 2 ('not yet implemented')."""
        # CLI11 may try to validate the file exists; supply /dev/null which
        # always exists and is readable.
        r = self._run("/dev/null", expect_returncode=2)
        self.assertIn("not yet implemented", r.stderr.lower())
