...
private:
    //! \brief Stored diagnostic text; surfaced by `what()`.
...
private:
    //! \brief Stored diagnostic text; surfaced by `what()`.
    std::string message_;   // +0x08
    //! \brief Zero-based position of the offending argument in
    //!        the original call; retrieved via `argIndex()`.
...
private:
    //! \brief Function registry — maps each user-callable
    //!        waveform built-in name to a bound member-function
    //!        wrapper.
    //!
    //! Populated by the constructor with the canonical names
    //! (`"zeros"`, `"sin"`, ...) plus the four alias entries
    //! that are registered directly rather than via
    //! `aliasMap_` (see IF-202).  `call()` resolves the
    //! request through this map after alias substitution.
    ...
    //! \brief libc++ unordered_map trailing `max_load_factor`
    //!        slot for `funcMap_`; declared explicitly for
    //!        layout fidelity with the binary.
    float funcMap_maxLoadFactor_{1.0f};  // +0x20  (libc++ unordered_map internal)

    //! \brief Deprecated-name → current-name redirect table.
    //!
    //! `call()` looks up the requested name here first and,
    //! on match, emits a deprecation warning through
    //! `warningCallback_` before forwarding to the
    //! `funcMap_` entry under the canonical name.
    ...
    //! \brief libc++ unordered_map trailing `max_load_factor`
    //!        slot for `aliasMap_`; declared explicitly for
    //!        layout fidelity with the binary.
    float aliasMap_maxLoadFactor_{1.0f}; // +0x48  (libc++ unordered_map internal)

    //! \brief Set of waveform names already produced through
    //!        `call()`.
    //!
    //! Used by `getOrCreateWaveform()` to short-circuit
    //! cached lookups: when `name` is present, the existing
    //! `WaveformFront` is fetched via
    //! `wavetableFront_->getWaveformByFunDescr` instead of
    //! re-registering.
    ...
    //! \brief Wavetable front-end the generator registers
    //!        produced waveforms with.
    //!
    //! Captured at construction; the generator never mutates
    //! the pointer.
    // +0x68: Shared pointer to wavetable front-end
    std::shared_ptr<WavetableFront>                          wavetableFront_;    // +0x68

    //! \brief 8-byte padding slot between `wavetableFront_`
    //!        and `warningCallback_` reproduced for layout
    //!        fidelity with the binary.
    // +0x78: 8 bytes — padding between wavetableFront_ and warningCallback_
    uint64_t pad_78_{0};  // +0x78

    //! \brief Warning sink invoked for deprecated-alias
    //!        substitutions and other non-fatal generator
    //!        diagnostics.
    // +0x80: Warning callback (invoked for deprecated function aliases)
    std::function<void(std::string const&)>                  warningCallback_;   // +0x80

    //! \brief Cancellation hook propagated by
    //!        `Compiler::setCancelCallback`.
    //!
    //! No reader inside `WaveformGenerator` has been
    //! identified yet; the slot exists to let a future
    //! cooperative-cancellation point poll the callback
    //! without re-routing through `Compiler`.  See IF-210.
