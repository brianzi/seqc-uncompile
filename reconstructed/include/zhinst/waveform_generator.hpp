...
private:
    ...
    float field_20_;  // +0x20  (initialized to 1.0f, likely max_load_factor or amplitude scale)
    ...
    float field_48_;  // +0x48  (initialized to 1.0f)
    ...
    // +0x78: 8 bytes — unknown / padding
    uint64_t field_78_;  // +0x78

    // +0x80: Warning callback (invoked for deprecated function aliases)
    std::function<void(std::string const&)>                  warningCallback_;   // +0x80

    // +0xB0: Initially null shared_ptr — Phase 15a confirmed NO setter
    // exists anywhere in the binary. See class-header comment above.
    std::shared_ptr<void>                                    field_B0_;          // +0xB0
};
