# Incidental Findings — Archive (IF-201..IF-300, closed entries)

Closed entries (`fixed`, `dismissed`, `confirmed-*`, `superseded`, `closed`) from
IF-201..IF-300, moved out of the live `incidental_findings.md` to keep it lean.
Open entries from this range remain in the live file.
...
# Additional closures (2026-05-16 archive pass 2)

The following entries were moved from the live `incidental_findings.md` after a second-pass
classification audit found that their bodies contained closure-status language (`fixed`,
`confirmed`, `deferred-by-design`, `documented`, `tracking-only`, etc.) even when the
first-line `**Status**:` field was not a single closed-status keyword.  IF-104 was also
promoted to archive (`confirmed-fixed-cosmetic`).
