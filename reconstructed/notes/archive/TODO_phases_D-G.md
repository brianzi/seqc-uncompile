# TODO archive — Phases D, E, F, G

Closed sub-tasks for Phase D (inline code documentation), Phase E
(diff-test harness for binding-unreachable reconstructions), Phase F
(backlog hardening), and Phase G (`\binarynote` re-audit).

All sub-tasks listed here are complete.  See `TODO.md` for active
work.  The narrative summaries of each phase live in `OVERVIEW.md`.
