# Incidental Findings — Archive (IF-100..IF-200, closed entries)

> **Archived 2026-05-12** during D15 cleanup.  These entries are all
> closed (status: `fixed`, `dismissed`, `kept`, `confirmed`, `resolved`,
> or `stale/unverifiable`).  Active entries from this ID range that
> remain open (IF-100, IF-102, IF-104) stay in the live
> `reconstructed/notes/incidental_findings.md`.
>
> Entries here are historical reference for cross-references from source
> comments and from `OVERVIEW.md` / `TODO.md` narratives.  They are not
> referenced as live action items.
