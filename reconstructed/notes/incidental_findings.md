...
- **Severity**: suspicious
- **Status**: **partially-fixed** (Phase R) — rename complete, type-fix deferred
...
- **Phase R resolution**:
  - **Rename done**: the field is now named `direction` (not `valueType`) at `expression.hpp:125` (`int32_t direction; // +0x54`). The header comment at line 112 documents `(EDirection enum; always 2 in binary)`.
  - **Type-fix deferred**: changing `int32_t` → `EDirection` (which is `enum class : int32_t` per `types.hpp:67`) would require touching ~30 literal-integer assignment sites (`->direction = 0;` / `= 2;` in `expression.cpp` and 12 sites in the autogenerated `seqc_parser.tab.c` which is regenerated from `seqc_parser.y`). Strict `enum class` typing forbids implicit int conversion, so each site would become `EDirection::eIN` / `EDirection::eINOUT`. Pure-cosmetic risk vs. churn trade-off; deferred to a future style pass. Consumer at `seqc_ast.cpp:43` already does the explicit cast: `static_cast<EDirection>(e.direction)`.
