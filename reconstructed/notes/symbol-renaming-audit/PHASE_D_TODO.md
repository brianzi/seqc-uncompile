...
- [ ] **Commit 1** — settle `nm` re-checks (~20 verify-not-original symbols, no source edits, audit notes only). **This commit closes the audit.**
- [ ] **Commit 2** — Cluster H (`clone` → `doClone`, base + ×32+ subclass overrides). Mechanical, vtable-atomic.
- [ ] **Commit 3** — Cluster F (`SeqCAstNode::type` → `lineNr` cascade, ×54).
