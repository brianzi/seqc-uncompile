...
// ============================================================================
// getUsedChannels — OR together ~channelMask from all usage entries
// 0x1df2f0
//
// UsageEntry elements at +0xE0 vector, each 0x20 bytes.
// Field at element+0x08 = channelMask (int).
// Returns bitwise OR of NOT(channelMask) across all entries.
// ============================================================================
uint32_t Prefetch::getUsedChannels() const {  // 0x1df2f0
    uint32_t result = 0;
    const char* begin = reinterpret_cast<const char*>(usageEntries_.data());
    const char* end = begin + usageEntries_.size() * 0x20;

    for (const char* p = begin; p != end; p += 0x20) {
        uint32_t channelMask = *reinterpret_cast<const uint32_t*>(p + 0x08);
        result |= ~channelMask;
    }
    return result;
}
...
// ============================================================================
// getUsedFourChannelMode — check if any usage entry has is4Channel set
// 0x1df400
//
// Field at element+0x0C = is4Channel (bool).
// ============================================================================
bool Prefetch::getUsedFourChannelMode() const {  // 0x1df400
    ...
    for (const char* p = begin; p != end; p += 0x20) {
        if (*reinterpret_cast<const bool*>(p + 0x0C)) {
            return true;
        }
    }
    return false;
...
// ============================================================================
// getUsedCache — recursively compute total cache usage for a node subtree
// 0x1c7eb0
//
// - For Load nodes (type==1): look up waveform, compute pages used
...
int Prefetch::getUsedCache(std::shared_ptr<Node> node) const {  // 0x1c7eb0
    ...
    // Try to get waveform name from wavesPerDev[deviceIndex]
    auto getName = [&]() -> std::optional<std::string> {
        int devIdx = n->deviceIndex;  // Node+0x40
        if (devIdx < 0) return std::nullopt;
        auto& wavesPerDev = n->wavesPerDev;  // Node+0x28
        if (static_cast<size_t>(devIdx) >= wavesPerDev.size()) return std::nullopt;
        auto& opt = wavesPerDev[devIdx];
    ...
    // Load node (type == 1)
    {
        ...
        if (name.has_value() && n->refCount == 1) {  // Node+0x44
            if (n->length != 0) {  // Node+0x90
                ...
                if (it == nodeStates_.end()) return 0;
                int pageSize = it->second.pageSize;
                return (n->length * channels) / pageSize;
            } else {
                auto wfm = wavetableIR_->getWaveformByName(name);
        ...
        if (name.has_value() && n->refCount == 2) {  // Node+0x44
            ...
            if (it != nodeStates_.end() && it->second.counter != 0) {
                if (n->length != 0) {
                    ...
                    uint16_t channels = wfm->signal.channels_;
                    int pageSize = it->second.pageSize;
                    return (n->length * static_cast<int>(channels)) / pageSize;
                } else {
                    auto wfm = wavetableIR_->getWaveformByName(name);
                    int memBytes = computeWaveformMemoryBytes(wfm.get());
                    int pageSize = it->second.pageSize;
    ...
    int result = 0;
    if (n->next) {
        result += getUsedCache(n->next);
    }
    if (n->loop) {
        result += getUsedCache(n->loop);
    }
    for (auto const& child : n->branches) {
        if (child) {
...
// ============================================================================
// getMemoryHighWatermark — max waveform memory footprint across devices
// 0x1cc650
//
// For HDAWG with numChannelGroups >= 2: iterates all channel groups.
// Otherwise: single device index.
...
uint32_t Prefetch::getMemoryHighWatermark() const {  // 0x1cc650
    ...
    if (numChannelGroups >= 2 && config_->deviceType == AwgDeviceType::HDAWG) {
        startIdx = 0;
        endIdx = static_cast<size_t>(numChannelGroups);
    } else {
        startIdx = config_->deviceIndex;  // +0x24
        endIdx = startIdx + 1;
    }
...
// ============================================================================
// getRequiredMemory — total waveform memory needed across devices
// 0x1cc930
//
// Same device iteration logic as getMemoryHighWatermark.
...
uint32_t Prefetch::getRequiredMemory() const {  // 0x1cc930
    ...
    for (size_t devIdx = startIdx; devIdx < endIdx; ++devIdx) {
        ...
        uint32_t deviceTotal = 0;
        for (auto const& wfm : waves) {
            int memBytes = computeWaveformMemoryBytes(wfm.get());
            deviceTotal += static_cast<uint32_t>(memBytes);
        }

        if (deviceTotal > maxUsage)
            maxUsage = deviceTotal;
    }
